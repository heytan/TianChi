...
## 总结：  
经过新人赛O2O项目对整个机器学习，尤其是特征工程，以及pandas的操作加深了理解。其中有一些值得注意的点：  
1.pd.read_csv(keep_default_na=False)这个参数会对数据中空值的处理产生影响。有这个参数时，空值是'null'字符串;没有这个参数，空值为NaN 浮点型。空值一般会被过滤掉，在统计等各种应用下。
2.read_csv(parse_dates = ['col_name'])可以在读取时就将这列转化为时间类型来处理。
时间类型的空值为NaT.  时间类型可以直接与相同格式的字符串做比较：datetime < '20160201'  
3.pd.merge(df1,df2,on='key', how='left')在提取合并特征时，常用左连接（how='left'）,是以df1的key为标准合并的.
合并完成后，需要注意产生的空值的填补。
